...
namespace RockersUnite.Controllers
{
    public class ArtistController : Controller
    {
        ...
        {
            string connection = ConfigurationManager.ConnectionStrings["RockersUnite"].ConnectionString;
            _ArtistDataAccess = new ArtistDataAccess(connection);
        }
        // Dependency Injection Refactor Here
        ...
        public ActionResult ViewAllArtists()
        {
            ...
            if (Session["Email"] != null || Session["Email"] == null)
            {
                try
                {
                    ...
                    List<IArtistDO> allArtists = _ArtistDataAccess.ViewAllArtists();
            ...
            if (Session["Email"] != null && (int)Session["Role"] >= 4)
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        _ArtistDataAccess.AddArtist(lArtistForm);
        ...
        public ActionResult UpdateArtist(int artistID)
        {
            ...
            if (Session["Email"] != null && (int)Session["Role"] >= 4)
            {
                ...
                IArtistDO artistDO = _ArtistDataAccess.ViewArtistByID(artistID);
            ...
            if (Session["Email"] != null && (int)Session["Role"] >= 4)
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        _ArtistDataAccess.UpdateArtistInformation(lArtistForm);
                        oResponse = RedirectToAction("ViewAllArtists", "Artist");
                    }
                    catch (Exception ex)
                    {
        ...
        public ActionResult RemoveArtist(int artistID)
        {
            ...
            if (Session["Email"] != null && (int)Session["Role"] >= 4)
            {
                try
                {
                    ...
                    _ArtistDataAccess.RemoveArtistByID(artistID);
                }
                catch (Exception ex)
                {
        ...
        public ActionResult Details(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    IArtistDO iArtist = (IArtistDO)_ArtistDataAccess.ArtistDetails(id);
